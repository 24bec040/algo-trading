...
scp -i "$KEY" -o StrictHostKeyChecking=no bot.py delta_client.py engine.py trades.py config.py logger.py ichimoku_bot.py ichimoku_config.py requirements.txt deploy.sh "$USER@$IP:~/trade-bot/"
if [ $? -ne 0 ]; then
